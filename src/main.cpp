...
#if defined(NDEBUG)
# error "Sonycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x74e121e4ec491468203eca4bd7ec3b56305092fa4638cf8d059de6fd7cdf40fa");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sonycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sonycoin Signed Message:\n";
...
{
    // Sonycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Sonycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Sonycoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Sonycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sonycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sonycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf6d79c87fc7148e6401c21c88b146a04f4a88ccb293ea7dc623a6e03cc947077");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Sept 26 2021 - Blue Jays win 5-2 over Twins";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b1f325b6097008db72fce0e91d59d1c608e5f9f1c1b79b23784f9b0f06d14db9067d88fd65125029a9348e72062e6ec10056eefd1eafb238e1b016a65cdcb4bc") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1632702849;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085186119;

        if (fTestNet)
        {
            block.nTime    = 1632702790;
            block.nNonce   = 386585287;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe69d92b7f432598416681c640d640766573a9c816272c3cec23e589ff356ae04"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc6, 0xb0, 0xdc }; // Sonycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SonycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SonycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SonycoinMiner(CWallet *pwallet)
{
    printf("SonycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sonycoin-miner");
    ...
    try { loop {
        ...
        printf("Running SonycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SonycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SonycoinMiner, pwallet));
}
